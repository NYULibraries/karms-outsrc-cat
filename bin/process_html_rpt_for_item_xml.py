...
batch_name = raw_input('Enter the full batch name (e.g., tp_20160121_bk_gre_14r): ')
...
batch_folder = parent_dir+'/submissions/'+vendor+'/'+batch_name+'/'+batch_stage+'-files/'

current_timestamp = time.strftime('%B %d, %Y%t%I:%M:%S %p', time.localtime())

# INPUT FILE(S)
subm_xml = minidom.parse(batch_folder+batch_name+'_'+batch_type+'_submission.xml')
...
batch_barcodes_file = codecs.open(batch_folder+batch_name+'_'+batch_type+'_barcodes', 'r')
